...
### python extract_introns.trinity.py BED_File Assembly_File Output_File [Subset]
        ...
        for j in junction_data:
            if j[0] == name:
                if j[4] >= 1 and j[7] != "?":
                    ...
                    intron_seq = junction_seq[int(block_locations[0])+int(block_sizes[0]) : int(block_locations[1])]
                    output_introns.write(">" + j[3].replace("JUNC", "INTRON") + " Score: " + str(j[4]) +"\n")
                    if j[7] == "-":
                        output_introns.write(intron_seq[::-1] + "\n")
                    else:
                        output_introns.write(intron_seq + "\n")